...
//static int unmask_hwpte(pte_t*, unsigned long); // declaration
//pte_t *pte_lookup(struct mm_struct*, int, unsigned long);
//void patch_instr(struct refcount* , unsigned long, struct mm_struct *, struct pt_regs *);
...
#if 0    /* simple heuristics. you can go fancy of course */
...
//      mm = current->mm;
        ...
        if (strncmp(tsk->comm, "xzltestprog", TASK_COMM_LEN) == 0) {
            ...
            if (pte) {
                printk(KERN_EMERG "[DEBUG] @ do_page_fault: I am here! 1\n");
                hpte = readl(hw_pte(pte));
                printk("[DEBUG] hpte = %08x.\n", (u32)hpte);

                if (hpte && (!(hpte & 0x3))) {
                    printk(KERN_EMERG "[DEBUG] @ do_page_fault: I am here! 2\n");
                    ...
                    if (tsk->refcount_head == NULL) {
                        printk(KERN_EMERG "[DEBUG] @ do_page_fault: I am here! 3\n");
                        if (NULL == (newref = 
                        ...
                        printk(KERN_EMERG "[DEBUG] @ do_page_fault: I am here! 4\n");
...
//                                                }
                    ...
                    printk("===> @ do_page_fault: Woo-hoo! caught an access. refcount=%d for addr = %08lx, pte(%08lx)\n", cref, addr,(unsigned long)*pte);
...
static int unmask_hwpte(pte_t* pte, unsigned long hpte)
{
	if (!hpte) {
		printk("[ERROR] Original hpte should not be 0!\n");
	}
    printk("[DEBUG] Original hpte was %08x. going to enable it.\n", (u32)hpte);

    writel(hpte, hw_pte(pte));
    clean_dcache_area((void *)(hw_pte(pte)), sizeof(pte_t)); /*bring TLB in sync. no $inv*/
    //flush_tlb_kernel_page(__phys_to_virt(hpte & PAGE_MASK)); /* necessary? */
    return 1;
}
...
//      mm = current->mm;
        ...
        if (strncmp(tsk->comm, "xzltestprog", TASK_COMM_LEN) == 0) {
            pte = pte_lookup(mm, 1, addr);
            if (pte) {
                printk(KERN_EMERG "[DEBUG] I am here! 1\n");
                hpte = readl(hw_pte(pte));
                printk("[DEBUG] hpte = %08x.\n", (u32)hpte);

                if (hpte && (!(hpte & 0x3))) {
                    printk(KERN_EMERG "[DEBUG] I am here! 2\n");
                    /* hw pte has bit set, but invalid. seems planted by us */
                    //printk("===> Woo-hoo! caught an access. refcount=%d\n", ++ref);
                                        
                    /* Yiyang: update refcounts link list in mm_struct */
                    if (tsk->refcount_head == NULL) {
                        printk(KERN_EMERG "[DEBUG] I am here! 3\n");
                        if (NULL == (newref = 
                                    (struct refcount *)kzalloc(
                                    sizeof(struct refcount), GFP_KERNEL))) {
                            printk(KERN_EMERG "[ERROR] kzalloc() failed. 1\n");
                        ...
                        printk(KERN_EMERG "[DEBUG] I am here! 4\n");
                        /* search the link list to find the right pte */
                        iter = tsk->refcount_head;
                        while (iter != NULL) {
//                            if (iter->vaddr == addr) {
...
//                            }
                            if (*(iter->pte) == *pte) {
                                ...
                                cref = iter->n;
                                refc = iter;
								saved_hpte = iter->hpte; //Yiyang: restore the original hpte
                                break;
                            }
                            iter = iter->next;
                        }
                        /* didn't find the vaddr in the link list, kzalloc a new one */
                        if (iter == NULL) {
							printk(KERN_EMERG "[DEBUG] It shouldn't be here\n");
                            if (NULL == (newref = 
                                        (struct refcount *)kzalloc(
                                        sizeof(struct refcount), GFP_KERNEL))) {
                                printk(KERN_EMERG "[ERROR] kzalloc() failed. 2\n");
...
//      mm = current->mm;
        ...
        if (strncmp(tsk->comm, "xzltestprog", TASK_COMM_LEN) == 0) {
            pte = pte_lookup(mm, 1, addr);
            if (pte) {
                printk(KERN_EMERG "[DEBUG] @ do_PrefetchAbort: I am here! 1\n");
                hpte = readl(hw_pte(pte));
                printk("[DEBUG] hpte = %08x.\n", (u32)hpte);

                if (hpte && (!(hpte & 0x3))) {
                    printk(KERN_EMERG "[DEBUG] @ do_PrefetchAbort: I am here! 2\n");
                    /* hw pte has bit set, but invalid. seems planted by us */
                    //printk("===> Woo-hoo! caught an access. refcount=%d\n", ++ref);
                                        
                    /* Yiyang: update refcounts link list in mm_struct */
                    if (tsk->refcount_head == NULL) {
                        printk(KERN_EMERG "[DEBUG] @ do_PrefetchAbort: I am here! 3\n");
                        if (NULL == (newref = 
                                    (struct refcount *)kzalloc(
                                    sizeof(struct refcount), GFP_KERNEL))) {
                            printk(KERN_EMERG "[ERROR] kzalloc() failed. 1\n");
                        ...
                        printk(KERN_EMERG "[DEBUG] @ do_PrefetchAbort: I am here! 4\n");
                        /* search the link list to find the right pte */
                        iter = tsk->refcount_head;
                        while (iter != NULL) {
//                            if (iter->vaddr == addr) {
...
//                                                }
                    ...
                    printk("===> @ do_PrefetchAbort: Woo-hoo! caught an access. refcount=%d for addr = %08lx, pte(%08lx)\n", cref, addr,(unsigned long)*pte);
                    #if 0 /* Yiyang: test: walk through the refcount link list */
                        printk("[DEBUG] Walk through the refcount link list\n");
                        iter = tsk->refcount_head;
                        while (iter != NULL) {
