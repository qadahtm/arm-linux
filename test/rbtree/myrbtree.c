...
red_blk_tree* red_blk_create_tree (int (*compare)(const void*, const void*), 
	...
	newTree = (red_blk_tree*)malloc(sizeof(red_blk_tree));
	if (newTree == NULL) {
		fprintf(stderr, "[ERROR] malloc() failed when allocating new tree.\n");
		abort();
	}
	...
	nil = (red_blk_node*)malloc(sizeof(red_blk_node));
	if (nil == NULL) {
		fprintf(stderr, "[ERROR] malloc() failed when allocating sentinel.\n");
		abort();
	}
	newTree->nil = nil;
	...
	root = (red_blk_node*)malloc(sizeof(red_blk_node));
	if (root == NULL) {
		fprintf(stderr, "[ERROR] malloc() failed when allocating root.\n");
		abort();
	}
	newTree->root = root;
...
red_blk_node* red_blk_search (void* key, red_blk_tree* tree) {
	red_blk_node* current = tree->root->left_child;
	red_blk_node* nil = tree->nil;

	if (current == nil)
		return nil;
	while (current != nil && 0 != tree->compare(key, current->key)) {
		/* search left child */
...
red_blk_node* red_blk_insert (void* key, red_blk_tree* tree) {
	red_blk_node* newNode = (red_blk_node*)malloc(sizeof(red_blk_node));
	red_blk_node* nil = tree->nil;

	if (newNode == NULL) {
		fprintf(stderr, "[ERROR] malloc() failed when allocating newNode.\n");
		abort();
	}
	newNode->key = key;
	newNode->red = 1;
...
void red_blk_delete_node (red_blk_tree* tree, red_blk_node* node) {
	...
	if (node->left_child != nil && node->right_child != nil) {
		...
		if (suc == nil) {
			fprintf(stderr, "[ERROR] No successor found!\n");
			abort();
		}
		node->key = suc->key;
